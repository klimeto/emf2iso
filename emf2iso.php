...
$geoserverURL = 'https://data.lter-europe.net/geoserver';
$geoserverPass = "admin:hFAvuw6km";

//header('Content-Type: text/xml');

$emfHarvestListXML = "https://data.lter-europe.net/deims/emf/harvest_list";
$file= simplexml_load_file($emfHarvestListXML);
$arr = json_decode(json_encode($file), true);
$sites = $arr['site'];
$siteUUID = $sites[0]["UUID"];
$siteEmfXMLUrl = $sites[0]["path"];
$siteEmfArray = xmlToArray(simplexml_load_file($siteEmfXMLUrl));
$siteEmfJSON = json_encode($siteEmfArray);
echo $siteEmfJSON;
exit();
foreach($sites as $site){
	$siteUUID = $site["UUID"];
	$siteEmfXMLUrl = $site["path"];
	$siteEmfArray = json_decode(xmlToArray(simplexml_load_file($siteEmfXMLUrl)));
	var_dump($siteEmfJSON);
